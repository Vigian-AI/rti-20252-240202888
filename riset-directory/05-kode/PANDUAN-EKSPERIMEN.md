...
### Opsi A — Eksekusi Otomatis (Sangat Direkomendasikan)

Telah disediakan skrip python `run_benchmarks.py` yang otomatis menjalankan:
1. Pengecekan status MongoDB dan jumlah dokumen di dalamnya (jika belum di-import, skrip otomatis mengimpor data).
2. Menyalakan API framework, menunggu hingga API tersebut *healthy* secara otomatis.
3. Menjalankan load-test k6.
4. Mematikan API framework secara bersih sebelum masuk ke run berikutnya.
5. Mengulangi langkah-langkah di atas secara berurutan sesuai dengan urutan interleave 10 run di atas.
6. Otomatis mengekstrak metrik k6 menjadi `results/all_runs.csv`.
7. Otomatis menjalankan analisis statistik (Shapiro-Wilk, uji beda, Cohen's d) dan menyimpan ringkasannya ke `results/summary_stats.csv`.

**Cara Menjalankan:**
```bash
python scripts/run_benchmarks.py
```
*Catatan: Skrip ini memerlukan Docker Desktop aktif di host. Skrip akan mengecek dependensi lokal (seperti numpy, pandas, dan scipy), dan jika tidak ada pada host, ia akan secara otomatis menjalankan ekstraksi & analisis di dalam container Docker agar tidak mengotori host.*

---

### Opsi B — Eksekusi Manual (Alternatif)
